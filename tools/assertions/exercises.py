...
def assert_create_exercise_response(
    ...
    """
    logger.info("Verifying create exercise response matches request data")
...
def assert_exercise(actual: ExerciseSchema, expected: ExerciseSchema):
    ...
    :raises AssertionError: Если хотя бы одно поле не совпадает.
    """
    logger.debug(f"Comparing exercise data for id={expected.id}")
...
def assert_get_exercise_response(
    ...
    """
    logger.info(f"Verifying GET response for exercise id={create_response.exercise.id}")
...
def assert_update_exercise_response(
    ...
    :raises AssertionError: Если хотя бы одно поле не совпадает.
    """
    logger.info(f"Verifying update exercise response for id={response.exercise.id}")
...
def assert_exercise_not_found_response(response: InternalErrorResponseSchema):
    ...
    """
    logger.info("Verifying 'Exercise not found' error response")
...
def assert_get_exercises_response(
    ...
    logger.info(f"Verifying exercises list: expected={expected_count}, actual={actual_count}")
    ...
    with allure.step(f"Verify all {actual_count} exercises match created data"):
        for exercise in get_response.exercises:
            with allure.step(f"Exercise: {exercise.title}..."):
                logger.debug(f"Verifying exercise id={exercise.id}")
